...
export default function HomePage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="bg-sand-900 text-sand-100 py-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="mt-8 flex justify-center gap-4 text-sm">
            <span className="flex items-center gap-2 bg-sand-800 px-4 py-2 rounded-full">
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        <div className="relative max-w-lg mx-auto mb-12">
          <Search
            className="absolute top-1/2 -translate-y-1/2 end-4 text-sand-400 pointer-events-none"
            size={18}
          />
          <input
            type="text"
            ...
            placeholder="חיפוש לפי שם, יחידה או סיפור..."
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl border border-sand-200 overflow-hidden animate-pulse"
              >
          ...
          <>
            ...
            {filtered.length > 0 ? (
              ...
              <div className="text-center py-20 text-sand-400">
                ...
                <button
                  onClick={() => setQuery("")}
                  className="mt-4 text-sm text-sand-600 underline"
                >
                  נקה חיפוש
                </button>
              </div>
